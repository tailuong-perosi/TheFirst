...
module.exports._get = async (req, res, next) => {
  try {
    ...
    if (req.query.delivery_date) {
      req.query[`from_date`] = moment(req.query.delivery_date)
        .tz(TIMEZONE)
        .startOf('days')
        .format()

      req.query[`to_date`] = moment(req.query.delivery_date)
        .tz(TIMEZONE)
        .endOf('days')
        .format()
    }
      ...
      aggregateQuery.push({
        $match: {
          'order_details.slug_title': {
            $in: [
              new RegExp(
                `${removeUnicode(req.query.product_name, false).replace(
                  /(\s){1,}/g,
                  '(.*?)'
                )}`,
                'ig'
      ...
      aggregateQuery.push({
        $match: {
          'order_details.sku': {
            $in: [
              new RegExp(
                `${removeUnicode(req.query.product_sku, false).replace(
                  /(\s){1,}/g,
                  '(.*?)'
                )}`,
                'ig'
      ...
      aggregateQuery.push({
        $match: {
          'customer.slug_name': new RegExp(
            `${removeUnicode(req.query.customer_name, false).replace(
              /(\s){1,}/g,
              '(.*?)'
            )}`,
            'ig'
      ...
      aggregateQuery.push({
        $match: {
          'customer.phone': new RegExp(
            `${removeUnicode(req.query.customer_phone, false).replace(
              /(\s){1,}/g,
              '(.*?)'
            )}`,
            'ig'
      ...
      aggregateQuery.push({
        $match: {
          'employee.slug_name': new RegExp(
            `${removeUnicode(req.query.employee_name, false).replace(
              /(\s){1,}/g,
              '(.*?)'
            )}`,
            'ig'
...
module.exports._create = async (req, res, next) => {
  try {
    ...
    if (!insert.insertedId) {
      throw new Error('500: Lỗi hệ thống, tạo nhật ký mua hàng thất bại!')
    }
    // try {
    //   let _action = {
