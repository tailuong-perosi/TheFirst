...
module.exports._get = async (req, res, next) => {
    try {
        ...
        if (req.query.label_id) {
            aggregateQuery.push({ $match: { label_id: Number(req.query.label_id) } });
        }
        if (req.query.code) {
        ...
        let [labels, counts] = await Promise.all([
            client.db(req.user.database).collection(`Labels`).aggregate(aggregateQuery).toArray(),
            client
...
module.exports._create = async (req, res, next) => {
    try {
        let insert = await client.db(req.user.database).collection(`Labels`).insertOne(req.body);
        ...
        try {
            let _action = {
                ...
                type: 'Tạo',
                properties: 'Nhóm cửa hàng',
                name: 'Tạo nhóm cửa hàng',
                data: req.body,
                performer_id: req.user.user_id,
                date: moment().tz(TIMEZONE).format(),
                slug_type: 'tao',
                slug_properties: 'nhomcuahang',
                name: 'taonhomcuahang',
            };
...
module.exports._update = async (req, res, next) => {
    try {
        await client.db(req.user.database).collection(`Labels`).updateOne(req.params, { $set: req.body });
        try {
            let _action = {
                business_id: req.user.business_id,
                type: 'Cập nhật',
                properties: 'Nhóm cửa hàng',
                name: 'Cập nhật nhóm cửa hàng',
                data: req.body,
                performer_id: req.user.user_id,
                date: moment().tz(TIMEZONE).format(),
                slug_type: 'capnhat',
                slug_properties: 'nhomcuahang',
                name: 'capnhatnhomcuahang',
            };
